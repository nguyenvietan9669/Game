...
(function (pixi_spine) {
    ...
    function isBuffer(resource) {
        return resource.xhrType == PIXI.loaders.Resource.XHR_RESPONSE_TYPE.BUFFER;
    }
    PIXI.loaders.Resource.setExtensionXhrType('skel', PIXI.loaders.Resource.XHR_RESPONSE_TYPE.BUFFER);
    function atlasParser() {
        return function atlasParser(resource, next) {
            ...
            var filePath = resource.name.replace(/_/g,'/');
            var baseUrl = `data/${filePath.split('/')[0]}`

            atlasPath = Utils.OMSGetData(filePath + '.atlas');
            Resource.GetAssetUrl(atlasPath, 'blob')
            .then(response =>
            {
                if (response instanceof Blob)
                {
                    response = Resource.CreateObjectURL(response);
                }
                ...
                if (resource.metadata && resource.metadata.atlasRawData) {
                    ...
                    this.add(resource.name + '_atlas', response, atlasOptions, function (atlasResource) {
    ...
    function imageLoaderAdapter(loader, namePrefix, baseUrl, imageOptions) {
        ...
        return function (line, callback) {
            ...
            if (cachedResource) {
                ...
                Resource.GetAssetUrl(url, 'blob')
                .then(url =>
                {
                    if (url instanceof Blob)
                    {
                        url = Resource.CreateObjectURL(url);
                    }
    ...
    if (PIXI.loaders.Loader) {
        ...
        if(GameConfig.isUseNewLoader){
            for(let newLoader of GameConfig.newLoaderArr){
                newLoader.use(atlasParser())
            }
        }
    }
})(pixi_spine || (pixi_spine = {}));
