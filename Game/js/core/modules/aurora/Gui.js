const BReader			= require('./BReader');
// const GLText			= require('./GLText');
const GLSprite			= require('./GLSprite');
const GLAnimation		= require('./GLAnimation');
const GLSpriteManager	= require('./GLSpriteManager');
const GuiDefine			= require('./GuiDefine');
...
class Gui extends PIXI.Container
{
	...
	{
		super();
		this.customTemplateLoader = null;
	}

	SetCustomTemplateLoader(func)
	{
		this.customTemplateLoader = func
	}
		...
		while (offset < data.length)
		{
			...
			else if (template == GuiDefine.TEMPLATE_GUI_TEXTITEM)
			{
				...
				let style = new PIXI.TextStyle({
					align: textAlign,
					fontFamily: font,
					fontSize: Utils.GetFontSize(size, font),
					fill: color,
					dropShadow: shadow,
			...
			else 
			{
				...
				for (let i = GUI_OBJECTS; i <= GUI_MODULE_8; i++) {
					let gui = new Gui();

					gui.SetCustomTemplateLoader(this.LoadCustomGui.bind(gui))
				...
				GLPack.Close();
				*/
				if (this.customTemplateLoader){
					offset = this.customTemplateLoader(template, posx, posy, anchor, data, offset)
				}
			}
		}
	}
